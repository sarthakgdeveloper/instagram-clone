      ...
      <div className="userPost__PopUpContainer">
        <div className="userPost__PopUpImageContainer">
          ...
          <div className="userPost__PopUpInfo">
                ...
                >
                  <i
                    className={`fa fa-heart ${
                      userPost.likes.includes(currentUser.userName)
                        ? "liked"
                        : null
                    }`}
                  ></i>
            ...
            <div className="popUp_Likes">{userPost.likes.length}</div>
          </div>
        </div>
        <div className="userPost__PopUpCommentContainer">
          <div className="userPost__popUpCaption">
            <p>
              {`${userPost.userName} ${
                userPost.caption ? `: ${userPost.caption}` : ""
              }`}
            </p>
          </div>
          <div className="userPost__PopUpComments">
            {userPost.comments
              ? userPost.comments.map((comment, index) => (
                  <div
                    key={`${userPost.uid}_${index}`}
                    className="userPost__EachComment"
                  >
                    <p className="userPost__comments">
                    ...
                    {comment.userName === currentUser.userName ? (
                      <span
                        ...
                        className="userPost__CommentDelete"
                      >
          ...
          <div className="userPost__PopUpInputComment">
            ...
            >
              <input
                ...
                value={newComment}
              />
              <button type="submit" className="comment__postButton">
                Post
              </button>
            </form>
          </div>
        ...
        <div className="userPost__PopUpCloseOption">
          <span onClick={handlePopUpClose}>
            <i className="fas fa-times userPost__PopUpClose"></i>
          </span>
          {user === "current" && (
            <div>
              <span onClick={handleDeleteOptionChange}>
                <i className="fas fa-ellipsis-v userPost__PostDelete"></i>
              </span>
              <div
                className={`post__Delete ${
                  deleteOption ? "DeleteOption__Show" : ""
                }`}
              >
                <p onClick={() => removePost(userPost, currentUser.userName)}>
                  Delete Post
                </p>
              </div>
            </div>
...
const mapDispatchToProps = (dispatch) => ({
  addLike: (post, currentUser, user) =>
    dispatch(incrementLike(post, currentUser, user)),
  addComment: (post, comment, currentUser, user) =>
    dispatch(getNewComment(post, comment, currentUser, user)),
  deleteComment: (post, comment, user) =>
    dispatch(removeComment(post, comment, user)),
