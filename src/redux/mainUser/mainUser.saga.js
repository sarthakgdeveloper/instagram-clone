...
function* signUpStart({ payload: { email, password, cPassword, cEmail } }) {
  ...
  if (cEmail === email && cPassword === password) {
    const emailExist = yield checkForExistingEmail(email);
    if (emailExist) {
...
function* userInfoEntered({
  payload: { email, password, displayName, userName, Bio },
}) {
  try {
...
export function* mainUserSaga() {
  yield all([
    ...
    call(isUserInfoEntered),
    call(onFollowUser),
    call(onUnFollowUser),
    call(onGetUserPost),
    call(onupdateNotification),
  ]);
}
